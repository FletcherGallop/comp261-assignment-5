...
//		assertTrue(text.compareTo(huffDecoded) == 0);
	...
	@Test 
	void encodeWarAndPeace() throws UnsupportedEncodingException, IOException {
		String text = new String(Files.readAllBytes(Paths.get("data/war_and_peace.txt")), "ASCII"); 
		System.out.println("War and Peace Text Length: " + text.length());
		
		HuffmanCoding huff = new HuffmanCoding(text);		
		byte[] huffEncoded = huff.encode(text);
		
		System.out.println("Compressed Length: " + huffEncoded.length);
		System.out.println("Tree: " + huff.tree);
	}
	
	@Test
	void encodedTaisho() throws UnsupportedEncodingException, IOException {
		String text = new String(Files.readAllBytes(Paths.get("data/taisho.txt")), "ASCII"); 
		System.out.println("Taisho Text Length: " + text.length());
	...
	@Test
	void encodedPi() throws UnsupportedEncodingException, IOException {
		String text = new String(Files.readAllBytes(Paths.get("data/pi.txt")), "ASCII"); 
		System.out.println("Pi Text Length: " + text.length());
